...
class PackingBoxesPairsUnseenColors(Task):
    ...
    def __init__(self):
        super().__init__()
        self.max_steps = 20
        # should have called it boxes :(
        self.lang_template = "pack all the {colors} blocks into the brown box"
    ...
    def reset(self, env):
        ...
        relevant_color_names = np.random.choice(
            all_color_names, min(2, len(bboxes)), replace=False)
        distractor_color_names = [
            c for c in all_color_names if c not in relevant_color_names]
        ...
        for bbox in bboxes:
            ...
            p.changeVisualShape(
                box_id, -1, rgbaColor=pack_colors[icolor] + [1])
        ...
        distractor_bbox_idxs = np.random.choice(
            len(bboxes), num_distractor_objects)
        for bbox_idx in distractor_bbox_idxs:
            ...
            if os.path.exists(urdf):
                os.remove(urdf)
            icolor = np.random.choice(
                range(len(distractor_colors)), 1).squeeze()
            if box_id:
                p.changeVisualShape(
                    box_id, -1, rgbaColor=distractor_colors[icolor] + [1])
