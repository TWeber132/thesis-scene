...
class PackingSeenGoogleObjectsSeq(task.Task):
    ...
    def reset(self, env):
        ...
        chosen_objs, repeat_category = self.choose_objects(
            object_names, len(bboxes))
        object_descs = []
        ...
        self.set_goals(object_descs, object_ids, object_points,
                       repeat_category, zone_pose, zone_size)
    ...
    def set_goals(self, object_descs, object_ids, object_points, repeat_category, zone_pose, zone_size):
        ...
        for obj_idx, (object_id, _) in enumerate(object_ids):
            ...
            self.lang_goals.append(
                self.lang_template.format(obj=object_descs[obj_idx]))
...
class PackingSeenGoogleObjectsGroup(PackingSeenGoogleObjectsSeq):
    ...
    def choose_objects(self, object_names, k):
        ...
        repeat_category, distractor_category = np.random.choice(
            chosen_objects, 2, replace=False)
    ...
    def set_goals(self, object_descs, object_ids, object_points, repeat_category, zone_pose, zone_size):
        ...
        self.goals.append((
            chosen_obj_ids, np.eye(
                len(chosen_obj_ids)), true_poses, False, True, 'zone',
